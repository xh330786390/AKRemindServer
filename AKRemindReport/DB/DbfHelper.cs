...
namespace AKRemindReport.DB
{
    ...
    public class DbfHelper
    {
        ...
        {
            if (string.IsNullOrEmpty(AkConfig.AkDir)) return null;

            string table = string.Empty;
            if (string.IsNullOrEmpty(date))
            {
                table = Path.Combine(AkConfig.AkDir, @"DATA\GNDSALE.Dbf");
            }
            else
            {
                table = Path.Combine(AkConfig.AkDir, date + @"\GNDSALE.Dbf");
            }

            if (!File.Exists(table)) return null;

            DataTable dt = null;
            DBFFile dbfFile = new DBFFile(table);
            try
            {
                dbfFile.Open();
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable OpenDbfFile()
        {
            return OpenDbfFile(string.Empty);
        }
    }
