...
namespace AKRemindServer.DB
{
    ...
    public class SqlServerHelper
    {
        ...
        public int ExecuteNonQuery(string connstr, string cmdText)
        {
            ...
            {
                if (!er.Message.Contains("未找到或无法访问服务器"))
                {
                    LogHelper.Error(typeof(SqlServerHelper) + ".ExecuteNonQuery Exception error=", er.Message);
                }
        ...
        {
            bool bl = false;
            string sql = string.Format("SELECT count(1)  FROM [master].[dbo].[sysdatabases] WHERE name='{0}'", DbName);
        ...
        /// <returns></returns>
        public bool ExistColumn(string connstr, string tableName, string columnName)
        {
            bool bl = false;
